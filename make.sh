...
if (( ! b_skipUnitTests )) ; then
    ...
    if [[ "$CONTAINER_ENV" =~ ^(docker|swarm)$ ]]; then
        if (( b_pfconInNetwork )) ; then
            if [[ $STORAGE_ENV == 'swift' ]]; then
                docker exec $pfcon_dev pixi run -e local pytest tests/test_resources_innetwork.py tests/test_resources_async_copy.py tests/test_swift_async_upload.py tests/compute --color=yes
                ...
                docker exec $pfcon_dev pixi run -e local pytest tests/test_resources_innetwork_fslink.py tests/test_resources_async_copy.py tests/compute --color=yes
            fi
        else
        ...
        if (( b_pfconInNetwork )) ; then
            if [[ $STORAGE_ENV == 'swift' ]]; then
                kubectl exec $pfcon_dev -- pixi run -e local pytest tests/test_resources_innetwork.py tests/test_resources_async_copy.py tests/test_swift_async_upload.py tests/compute --color=yes
            elif [[ $STORAGE_ENV == 'filesystem' ]]; then
                kubectl exec $pfcon_dev -- pixi run -e local pytest tests/test_resources_innetwork_fs.py tests/compute --color=yes
            elif [[ $STORAGE_ENV == 'fslink' ]]; then
                kubectl exec $pfcon_dev -- pixi run -e local pytest tests/test_resources_innetwork_fslink.py tests/test_resources_async_copy.py tests/compute --color=yes
            fi
        else
